...
class Trie:
    ...
    在计算机科学中，trie（/ˈtriː/, /ˈtraɪ/）又称数字树或前缀树，[1] 是一种 kary 搜索树，
    是一种用于从集合中查找特定键的树形数据结构。这些键通常是字符串，节点之间的链接不是由整个键定义的，
    而是由单个字符定义的。为了访问一个键（恢复键值、更改键值或删除键值），
    需要按照节点间的链接（代表键中的每个字符）以深度优先的方式遍历三叉树。
    """
